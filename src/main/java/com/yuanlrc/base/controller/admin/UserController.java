...
public class UserController {
	...
	public Result<Boolean> add(User user){
		...
		if(user.getRole().getId() == UserType.ADMIN.getCode()){
			if(user.getCommunity() == null || user.getRole().getId() == null){
				return Result.error(CodeMsg.COMMUNITY_NULL_ERROR);
			}
		}
		//到这说明一切符合条件，进行数据库新增
	...
	public String edit(Model model,@RequestParam(name="id",required=true)Long id){
		...
		model.addAttribute("admin", UserType.ADMIN.getCode());
